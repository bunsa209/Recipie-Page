# Recipie-Page
A recipie page but soon to be a cokbook website.
<!DOCTYPE html>
<html lang="en">
    <head>
        <title>C for Chef CookBook</title>
        <meta 
        name="Description"
        content="A WEBSITE WITH RECIPIES FOR BEGINNERS">
        <meta charset="utf-8">
    </head>
    <body>
        <center>
        <h1>C for Chef CookBook</h1>
        <p>This is a CookBook that is filled with easy-go-recipies for beginners in cooking who are welcoming themselves into the world of flavours and variety of tastebuds.</p>
        </center>
        <h2>Simple Chocolate cake</h2>
        <p>This is a very basic recipie of a Chocolate cake if you are craving for one but does not aim for anything fancy!</p>
        <img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEje9AebtKq_2enZOTEEsQwIXVzyorRGnvvwfpIcPISxkmtUYO34PBgONLGrkHnCtgdTHp7RyEio81pA8M8qat_3sZgitXvvanNbPT5ZElWAmWpcQEUfMlalb5MhfXv1gefAxCjVF6r_90GdNxX6QucuzdyahyDxEUfRYITGSd1KkFkdfQFMyJsUwbj6-w18/s1800/Chocolate%20Cake%20simple.jpeg"
        alt="image of a Chocolate cake">
        <h3>Ingredients</h3>
        <ol>
            <li>All-purpose flour - 1 cup</li>
            <li>Sugar - 1 cup</li>
            <li>Cocoa Powder - 1/4 cup</li>
            <li>Baking soda - 1/4 teaspoon</li>
            <li>baking powder - 1/2 teaspoon</li>
            <li>salt - 1/4 teaspoon</li>
            <li>Coffee powder - 1/2 teaspoon</li>
            <li>Butter Milk - 1/2 cup</li>
            <li>Oil - 1/2 cup</li>
            <li>Hot Milk - 1/2 cup</li>
            <li>Egg - 1</li>
        </ol>
        <h3>Alternates for ingredients</h3>
        <ul>
            <li>Vanilla Essence instead of Coffee Powder</li>
            <p>Coffee Powder definetly brings out the flavour of Chocolate cakes in a dense way but if you are a fan of vanilla flavour with Chocolate then you can give this a try , make sure to add just a small amount.</p>
            <li>Almond flour or any other gluten free flour instead of All-purpose</li>
            <p>If you are looking for some healthy diet options</p>
        </ul>
        <h3>Method of Cooking</h3>
        <ul>
            <li>Ingredients Preparation Time: 10 minutes</li>
            <li>Bake Time: 35 minutes</li>
        </ul>
        <ol>
            <li>Preheat Oven to 350F. </li>
            <li>Mix together all dry ingredients.</li>
            <li>In other bowl, mix in all of wet ingredients.</li>
            <li>Combine the dry and wet ingredients together.</li>
            <li>Line a cake baking tin with parchment paper or grease with Butter or oil. </li>
            <li>Bake for 30-40 minutes depending on the size of your cake tin. Make sure not to burn it. Keep an eye on the oven while baking.</li>
            <li>After 30 mins, instert a fork or toothpick at the centre to see if it comes out clean. If it is clean it means cake is cooked , if not keep it for 5 or 10 mins more.</li>
            <li>Cool for some time and enjoy with some hot coffee , whipping cream or any sweet sauces of youur choice.</li>
        </ol>
         <!--creating a form-->
        <h2>Feedback</h2>
        <p>Be Sure to leave a Feedback after you cook.</p>
        <form>
            <div class="form-group">
                <!--this is the division for name input and div is used for easy understanding , layout ans styling-->
                <label for="name">Name:</label>
                <input id="name" type="text" name="name" placeholder="Enter your name here">
            </div>
            <br>
            <div class="form-group">
                <label for="email">Email:</label>
                <input id="email" type="email" name="user-email" placeholder="Enter your email address here">
            </div>
            <br>
            <div class="form-group">
                <label>Recipe Rating</label>
                <label for="five">5</label>
                <input id="five" type="radio" name="recipe_rating">
                <label for="four">4</label>
                <input id="four" type="radio" name="recipe_rating">
                <label for="three">3</label>
                <input id="three" type="radio" name="recipe_rating">
                <label for="two">2</label>
                <input id="two" type="radio" name="recipe_rating">
                <label for="one">1</label>
                <input id="one" type="radio" name="recipe_rating">
            </div><br>
            <div class="form-group">
                <label>what did you like the most?</label>
                <label for="easy">Easiness</label>
                <input id="easy" type="checkbox" name="easy">
                <label for="texture">texture</label>
                <input id="texture" type="checkbox" name="texture">
            </div><br>
            <div class="form-group">
                <label>Tell Us More</label>
                <textarea id="comments" name="comments" placeholder="Enter your comments here feel free to open up" rows="10"></textarea>
            </div>
            <div class="form-group">
                <button type="submit" id="submit" name="submit">Submit</button>
            </div>
        </form>
    </body>
</html>
